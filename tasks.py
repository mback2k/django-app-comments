...
def notification_post_approval_pending(post_id):
    post = Post.objects.get(id=post_id, is_approved=True)

    perm = Permission.objects.get_by_natural_key(codename='change', app_label='comments', model='post')
    users = User.objects.filter(Q(groups__permissions=perm) | Q(user_permissions=perm)).distinct()

    for user in users:
        notification_post_approval_pending_user.delay(post.id, user.id)

@task(ignore_result=True)
def notification_post_approval_pending_user(post_id, user_id):
    post = Post.objects.get(id=post_id, is_approved=True)
